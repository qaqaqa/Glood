...
- (void)initialize{
    ...
    _scrollView.contentSize = CGSizeMake(_scrollView.frame.size.width*10000, _scrollView.frame.size.height);
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView;
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"startScrollViewIsScrolling" object:self];
}
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate;
{
    NSLog(@"scrollViewDidEndDragging");
    [[NSNotificationCenter defaultCenter] postNotificationName:@"endScrollViewIsScrolling" object:self];
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    switch (orientation) {
        case PagedFlowViewOrientationHorizontal:
            ...
            break;
        case PagedFlowViewOrientationVertical:
            pageIndex = round(MAX(_scrollView.contentOffset.y, 0) / _pageSize.height);
            break;
        default:
            break;
    }
    ...
    if ([_delegate respondsToSelector:@selector(flowView:didScrollToPageAtIndex:)] && _currentPageIndex != pageIndex) {
        NSLog(@"xxxxxxx------ %ld",(long)pageIndex);
